...
df = (
  ...
  .load("dbfs:/mnt/files/landing-zone/1988.parquet")
)
...
# MAGIC %sql
# MAGIC CREATE TABLE IF NOT EXISTS bronze_ingest.flight_details USING PARQUET LOCATION 'dbfs:/mnt/files/landing-zone/1988.parquet';
...
# MAGIC %sql
# MAGIC SELECT *
# MAGIC FROM bronze_ingest.flight_details;

# COMMAND ----------

# MAGIC %sql
# MAGIC COPY INTO bronze_ingest.flight_details
# MAGIC FROM 'dbfs:/mnt/files/raw/' FILEFORMAT = CSV FORMAT_OPTIONS('header' = 'true', 'sep' = '\t');
...
display(
  flight_details.filter("Cancelled == 1")
  .groupby("FlightNum")
  .count()
...
flight_df = flight_details.withColumn(
  "arrDelay", F.regexp_replace("arrDelay", "NA", "0")
...
flight_df = flight_df.withColumn(
  "arrDelay", flight_df.arrDelay.cast(IntegerType())
...
flight_df.groupby("FlightNum").agg(
